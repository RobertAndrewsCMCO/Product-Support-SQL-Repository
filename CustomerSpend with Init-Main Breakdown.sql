...
If(OBJECT_ID('tempdb..#MSPHDRS0') Is Not Null)
    ...
    WHERE CAST(try_convert(date,str(MSPHDRS0.IVDAT8),112) as date) >='2023-01-01'
...
/*SET Region By Manager*/
    ...
    WHERE CAST(try_convert(date,str(wophdrs0.IVDT8),112) as date) >='2023-01-01'
...
SELECT TRY_CONVERT(date, CAST(M.IVDAT8 AS CHAR(8)), 112) 
    ...
    --Filter for one month test
    -- WHERE IVDAT8 > 20250101 
    -- and M.CUNO = '029990'

--     SELECT DISTINCT M.RFDCNO, M.CUNO, M.STN1, M.ACCTNO, M.NET_SELL_AMT, M.DEALER_NET
-- FROM #MASTER_DATASET M
-- WHERE ISNUMERIC(M.RFDCNO) = 0
--    OR ISNUMERIC(M.CUNO) = 0
--    OR ISNUMERIC(M.STN1) = 0
--    OR ISNUMERIC(M.ACCTNO) = 0
--    OR ISNUMERIC(M.NET_SELL_AMT) = 0
--    OR ISNUMERIC(M.DEALER_NET) = 0;